...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap transition-[background-color,border-color,color] duration-120 ease-in-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-ring)] focus-visible:ring-offset-2 disabled:pointer-events-none disabled:cursor-not-allowed disabled:border-[var(--color-border-strong)] disabled:bg-[var(--color-disabled-bg)] disabled:text-[var(--color-disabled-text)] disabled:opacity-100 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        primary:
      ...
      size: {
        sm: "min-h-[30px] px-3 py-1.5 text-xs font-semibold rounded-[var(--radius-sm)]",
        md: "min-h-[34px] px-5 py-2 text-[13px] font-semibold rounded-[var(--radius-sm)]",
        lg: "min-h-[38px] px-6 py-2.5 text-sm font-semibold rounded-[var(--radius-md)]",
